/* ------ JavaScript - HTML Canvas Particles 3 - Interactive Trailing Cloud Particles ------ */
    ...
    if ( mouse.x - this.x < mouseRadius
        ...
        && mouse.y - this.y > -mouseRadius) {
            if (this.size < maxSize) {
                this.size += 5; // size of particle shape.
            }
        } else if (this.size > minSize) {
            this.size -= 0.5; // this controls the minimum particle size.
        }
        if (this.size < 0) {
            this.size = 0;
        }
...
function init() {
    particleArray = [];
    for (let i = 0; i < 3000; i++) { // this controls the amount of particles allowed.
        let size = 0;
